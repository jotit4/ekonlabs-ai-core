...
export async function POST(
  ...
  try {
    ...
    await client.request<{ status: string }>(
      `/api/v1/tenants/${tenantId}/conversations/${phone}/takeover`,
      { method: 'POST' }
    )

    // 6. Registrar audit log después del takeover exitoso
    await logAudit({
      action: 'conversation_takeover',
      entity_type: 'conversation',
      entity_id: phone,
    ...
    if (err instanceof FastAPIError) {
      if (err.status === 409) {
        // Otro usuario ya tomó control
        const body = err.body as { controlled_by?: string } | null
        return Response.json(
          { error: 'conflict', controlled_by: body?.controlled_by ?? 'otro operador' },
          { status: 409 }
        )
      }
      console.error('[takeover/POST] FastAPIError:', err.status, err.body)
    } else {
      console.error('[takeover/POST] error:', err)
    }
    return Response.json({ error: 'takeover_failed' }, { status: 503 })
  }
}
