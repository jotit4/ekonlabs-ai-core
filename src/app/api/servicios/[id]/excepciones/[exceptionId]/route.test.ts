...
function makeDeleteParams(id: string, exceptionId: string) {
  return { params: Promise.resolve({ id, exceptionId }) }
}
...
describe('DELETE /api/servicios/[id]/excepciones/[exceptionId]', () => {
  beforeEach(() => { vi.clearAllMocks() })

  it('retorna 401 sin usuario', async () => {
    mockGetUser.mockResolvedValue({ data: { user: null }, error: null })
    const res = await DELETE(new Request('http://localhost'), makeDeleteParams('svc-1', 'exc-1'))
  ...
  it('retorna 204 cuando la excepción existe y pertenece al tenant', async () => {
    setupAdminAuth()
    mockFrom.mockReturnValue(makeDeleteChain({ error: null }))

    const res = await DELETE(new Request('http://localhost'), makeDeleteParams('svc-1', 'exc-uuid-1'))
  ...
  it('retorna 500 si Supabase falla', async () => {
    ...
    const body = await res.json() as { error: string }
    expect(body.error).toBe('Error al eliminar la excepción')
  })
})
