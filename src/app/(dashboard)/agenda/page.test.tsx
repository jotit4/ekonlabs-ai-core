...
describe('AgendaPage (guard de rol)', () => {
  beforeEach(() => {
    // Default: sin profesional vinculado (no dispara el atajo "ver mi día").
  ...
  // Atajo "ver mi día" (pedido ISADI 2026-07-17)
  it('un admin vinculado a un profesional que entra "en frío" es redirigido a su día', async () => {
    mockGetUser.mockResolvedValue({ data: { user: { id: 'u1' } } })
    mockGetSession.mockResolvedValue({
      data: { session: { access_token: makeJwt('admin') } },
    })
    mockParseJwt.mockReturnValue({ app_role: 'admin', tenant_id: 'tenant-1' })
    mockSingle.mockResolvedValue({ data: { professional_id: 'prof-1' } })
  ...
  it('un admin vinculado NO es redirigido si ya hay query params (respeta su navegación)', async () => {
    mockGetUser.mockResolvedValue({ data: { user: { id: 'u1' } } })
    mockGetSession.mockResolvedValue({
      data: { session: { access_token: makeJwt('admin') } },
    })
